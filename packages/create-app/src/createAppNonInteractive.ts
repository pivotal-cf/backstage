...
export default async (opts: OptionValues): Promise<void> => {
  ...
  const appDir = opts.path
    ...
    : resolvePath(paths.targetDir, opts.name);
  ...
  try {
    ...
    if (opts.path) {
      ...
      await checkAppExistsTask(paths.targetDir, opts.name);

      Task.section('Creating a temporary app directory');
      const tempDir = await fs.mkdtemp(resolvePath(os.tmpdir(), opts.name));
      ...
      await moveAppTask(tempDir, appDir, opts.name);
    }
    ...
    Task.log();
    Task.log(chalk.green(`🥇  Successfully created ${chalk.cyan(opts.name)}`));
    Task.log();
    Task.section('All set! Now you might want to');
    if (opts.skipInstall) {
      Task.log(
        `  Install the dependencies: ${chalk.cyan(
          `cd ${opts.path ?? opts.name} && yarn install`,
        )}`,
      );
    }
    Task.log(
      `  Run the app: ${chalk.cyan(
        `cd ${opts.path ?? opts.name} && yarn dev`,
      )}`,
    );
    Task.log(
